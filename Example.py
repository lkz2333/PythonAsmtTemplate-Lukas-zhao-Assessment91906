...
#The .lower parameter is used to convert all letters to lowercase to prevent query failures due to case sensitivity.
...
def parse_line(line):
    ...
    # Extract the supermarket name from a split list like:
    # "Milk 2L | $4.80 | Countdown"
    # or "Milk 3L | New World | $8.10"
    
    parts = [part.strip() for part in line.split("|") if part.strip()]
    supermarket_candidates = []

    for part in parts:
        if "$" in part or re.search(r"\d+(?:\.\d+)?", part):
            continue
        supermarket_candidates.append(part)

    if supermarket_candidates:
        supermarket = supermarket_candidates[-1]
    else:
...
def search(query):
    ...
    if not lines:
        messagebox.showwarning(
            "No Data",
            "list.txt It's currently empty. Please enter one product per line in the file first.\n"
            "For example:Milk 2L - $4.80 - Countdown",
        )
        return
